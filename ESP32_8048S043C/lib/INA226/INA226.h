...
class INA226 {
   public:
    INA226(uint8_t addr) : _wire(nullptr), _addr(addr) {}
    ...
    float readCurrent() {
        ...
        // Read calibration to calculate Current_LSB
        uint16_t cal = readRegister(INA226_REG_CALIBRATION);
        if (cal == 0) return 0.0;

        // Current_LSB = 0.00512 / (CAL × R_shunt)
        // For R_shunt = 0.0002Ω
        float current_lsb = 0.00512 / (cal * 0.0002);

        return signed_raw * current_lsb;
    }
